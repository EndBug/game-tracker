import { DMChannel, TextChannel, MessageReaction, User, Message } from 'discord.js'
...
export default class SampleCMD extends Command {
  ...
  async run(msg: Message) {
    if (Object.keys(APIUtil.findAll(msg.author)).length == 0) return msg.reply('There\'s no stored data about you.')

    const { channel } = msg
    const perm1 = channel instanceof DMChannel || channel.permissionsFor(client.user).has('ADD_REACTIONS')
    const perm2 = channel instanceof TextChannel && channel.permissionsFor(client.user).has('MANAGE_MESSAGES')
    ...
    else main.reactions.cache.forEach(r => {
