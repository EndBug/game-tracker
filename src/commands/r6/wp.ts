...
export default class R6WPWiki extends Command {
  constructor() {
    super(getConfig('wp', {
      ...
      examples: {
        'camrs BandltlsMyMaln': 'Displays CAMRS stats for `BandltlsMyMaln` by searching the user in the `uplay` category.',
        'assault Truman xbl': 'Displays assault weapons stats for `Truman` by searching the user in the `xbl` category.',
        'L85A2 @DiscordUser': 'Displays L85A2 stats for the mentioned user, if they linked their account.',
