...
async function getLast(): Promise<any> {
  const attachment = channel.lastMessageID
    ? (await channel.messages.fetch(channel.lastMessageID)).attachments.first()
    : undefined
  if (attachment?.url) {
...
async function loadChannels(check = true) {
  for (const key in IDs) {
    ...
    if (
      !((tempC): tempC is TextChannel => tempC && tempC.type === 'text')(tempC)
    )
      return
    channels[key] = tempC
  }

  if (check) for (const c of Object.values(channels)) if (!c) return false
  ...
  return new Promise((resolve, reject) => {
    ...
    client.on('ready', async () => {
      ...
      if (!guild || !guild.available)
        return reject(
          `Guild '${guildID}' is ${guild ? 'unavailable' : 'undefined'}`
        )
      await loadChannels()
      channel = channels.backups
      if (!channel)
        return reject(`Channel '${IDs.backups}' does not exist in this guild`)
...
export async function restore(force = false) {
  if (exists() && !force) console.log('Trying to load database...')
    ...
    try {
      ...
      return console.log(
        `Unable to restore previous database, creating a new one. Error: \`\`\`\n${err}\n\`\`\``
      )
    }
}
  ...
  if (ready) {
    ...
    if (notify) {
      ...
      await upload('**CRASH**').then(() => {
        channels.backups.send(
          "Couldn't send notification message, check channel ID."
        )
      })
    }
